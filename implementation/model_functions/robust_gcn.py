...
class RobustGCNLayer(nn.Module):
    ...
    def __init__(self, adj: sp.spmatrix, dims, device):
        super().__init__()
        self.weights = nn.Parameter(nn.init.xavier_normal_(torch.zeros(dims, device=device)))
        self.bias = nn.Parameter(nn.init.normal_(torch.zeros(dims[1], device=device)))
        self.D, self.H = dims
        if adj.format != "csr":
            adj = adj.tocsr()

        self.adj = adj.astype("float32")
        self.adj_tensor = sparse_tensor(self.adj).to(device)
    ...
    def slice_adj(self, nodes):
        ...
        adj_slice = sparse_tensor(self.adj[nodes][:, nbs]).to(self.device)
...
class RobustGCNModel(nn.Module):
    ...
    def __init__(self, num_layers, dataset, device):
        ...
        dims = [num_initial_features] + hidden_dims + [num_final_features]

        self.name = 'ROBUST_GCN'
        ...
        adj = sp.csr_matrix((val, (row, col)), shape=(data.num_nodes, data.num_nodes)).astype("float32")
        ...
        for ix,hidden in enumerate(dims[1:]):
            self.layers.append(RobustGCNLayer(self.adj_norm, [previous, hidden], device).to(device))
    ...
    def forward(self, input=None, nodes=None):
        """
        ...
        input = torch.matmul(input, self.glove_matrix).to(self.device)
        # end of changes XXXXX
    ...
    def dual_backward(self, input, nodes, q, Q, target_classes=None,
        ...
        # [Batch, K, K]
        C_tensor = (predicted_onehot.unsqueeze(1) - torch.eye(self.K)).to(self.device)
        phis = [-C_tensor]

        # final_objective = torch.zeros([batch_size, self.K], device="cuda")
        bias_terms = torch.zeros([batch_size, self.K], device=self.device)
        I_terms = torch.zeros([batch_size, self.K], device=self.device)
                ...
                if initialize_omega:
                    ...
                    with torch.no_grad():
                        self.omegas[ix-1].index_put_((torch.LongTensor(neighborhoods[ix-1]),),
                                                     omega.cpu())
                omega = self.omegas[ix - 1][neighborhoods[ix - 1]].to(self.device)
            else:
                omega = None
...
def train(gcn_model, X, y, idx_train, idx_unlabeled, q, Q=12, n_iters=1000, method="Robust Hinge U", burn_in=100,
    ...
    attrs_dense = X#.to_dense() CHANGED XXXXX
        ...
        if it > burn_in and method != "Normal":
            for inner in range(margin_iters):
                ...
                if "Robust Hinge" in method:
                    ...
                    lower_bound_margins = relu(-lower_bound_margins + margin_term.to(gcn_model.device))
                    robust_loss = (lower_bound_margins.mean()) / margin_iters
                elif method == "Robust Cross Entropy":
                    robust_loss = ce_loss(-lower_bound_margins, torch.LongTensor(predicted).to(gcn_model.device))/margin_iters
                with torch.no_grad():
